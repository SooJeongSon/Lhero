...
{
	FileManager::GetInstance()->LoadIniFile("Text/tile_data.ini", "tile_data");

	map<string, map<string, string>> mDatas = FileManager::GetInstance()->GetData("tile_data");
	...
	for (auto group : mDatas)
	{
		...
		mLpTiles[group.first]->spawn = mDatas[group.first]["spawn_0"];
